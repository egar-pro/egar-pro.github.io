<html>
<body>
Firstname: <?php echo $_POST["firstname"]; ?><br>
Lastname: <?php echo $_POST["lastname"]; ?><br>
Email: <?php echo $_POST["email"]; ?><br>
Phone Number: <?php echo $_POST["tel"]; ?><br>
Location: <?php echo $_POST["location"]; ?><br>
Month: <?php echo $_POST["month"]; ?><br>
Day: <?php echo $_POST["day"]; ?><br>
Year: <?php echo $_POST["year"]; ?><br>
Nights: <?php echo $_POST["nights"]; ?><br>
Adults: <?php echo $_POST["adults"]; ?><br>
Children: <?php echo $_POST["children"]; ?><br>
Text-Area: <?php echo $_POST["subject"]; ?><br>
</body>
</html> 